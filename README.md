...
## Standalone testing (no Make.com)
Use the minimal test list to run independently, then hook Make later.
...
```bash
npm run test:local
# or a slightly larger run:
npm run test:urls
```

After it finishes, check:
- `out/test-results.ndjson` (or `out/results.ndjson`)
- Validate structure:
```bash
